...
from genesis_packages.manifest import load_manifest,hash_package
from genesis_packages.resolver import Resolver
from genesis_packages.lockfile import Lockfile,lock_from_resolution
from genesis_packages.store import ContentStore
from genesis_packages.errors import PackageError
from genesis_packages.semver import Requirement,Version
from genesis_packages.util import sha256_obj,write_json
from genesis_callables import parse_extended_source,compile_sources
from genesis_callables.util import parse_call_head
from genesis_callables.expander import Expander,_base_parse_line
...
@dataclass
class CallablePackageBuildResult:
    manifest: object; lockfile: Lockfile; callable: object; toolchain: dict; execution: object; receipt: dict; package_sources: list

class CallablePackageRuntime:
    ...
    def resolve(self,project_path,frozen_lock=None):
        ...
        if frozen_lock:
            ...
            return root,self._resolution_from_lock(root,lock),lock
        res=Resolver(self.registry).resolve(root); return root,res,lock_from_resolution(root,res)
    def _packages(self,root,res): return [(n,res['selected'][n].manifest) for n in res['order']]+[(root.name,root)]
    def audit(self,root,res):
        packs=self._packages(root,res); module_owner={}; modules={}; parsed={}
        for pn,m in packs:
            for md in m.modules:
                mn=md['name']
        ...
        for mn,(m,p) in modules.items():
            ast=parse_extended_source(p.read_text(encoding='utf-8'),str(p))
            if ast.module!=mn: raise PackageError(f'module declaration {ast.module} differs from manifest {mn}','MANIFEST_INVALID')
            parsed[mn]=ast
        exp=Expander(list(parsed.values()))  # validates callable visibility/import graph/recursion
        package_effects={}; package_caps={pn:sorted(set(m.backend_capabilities)) for pn,m in packs}
        for pn,m in packs:
            used=set(); allowed=set(m.dependencies)
            for md in m.modules:
                ast=parsed[md['name']]
                # base value imports + callable imports obey same direct-dependency rule.
                base_text='genesis 0.1.0\nmodule '+ast.module+' {\n'+'\n'.join('  '+s for _,s in ast.base_lines if not parse_call_head(s) and not s.startswith('export '))+'\n}\n'
                try:
                    from genesis_frontend import parse_source
                    bast=parse_source(base_text,ast.file)
                    for imp in bast.imports:
                    ...
                    for st in bast.statements: used |= effects_for_op(OPS[st.kind])
                except Exception as e:
                    if isinstance(e,PackageError): raise
                    raise PackageError(f'base syntax audit failed in {ast.module}: {e}','TOOLCHAIN_FAILURE')
                for fi in ast.function_imports:
                    owner=module_owner.get(fi.module)
                    if owner is None: raise PackageError(f'{pn} imports unknown callable module {fi.module}','MODULE_UNRESOLVED')
                    if owner!=pn and owner not in allowed: raise PackageError(f'{pn} imports transitive/non-direct callable module {fi.module} owned by {owner}','DIRECT_DEPENDENCY_REQUIRED')
                # Library code declares potential effects even when functions are not invoked during this package build.
                for fn in ast.functions: used |= set(fn.effects)
                # Top-level calls count against caller effect budget.
                for ln,line in ast.base_lines:
                    c=parse_call_head(line)
                    if c: used |= set(exp._resolve(ast.module,c[0]).effects)
    ...
    def build(self,project_path,frozen_lock=None,optimization='O1',profile='audit',execute=True):
        ...
        installed={n:str(self.store.install(res['selected'][n])) for n in res['order']}; audit=self.audit(root,res)
        ...
        cb=compile_sources(sources,exports=root.entry_exports or None,name=root.name)
        tool=optimize_sources(cb.expanded_sources,exports=root.entry_exports or None,name=root.name,level=optimization,profile=profile)
        ...
        pre={'kind':'GENESIS_CALLABLE_PACKAGE_BUILD_RECEIPT','version':'0.1.0','package':root.name,'package_version':root.version,'root_content_sha256':hash_package(root.root),'resolution_root':lock.resolution_root,'dependency_order':res['order'],'installed_content':installed,'source_modules':source_packages,'effect_audit':audit['effects'],'capability_declarations':audit['capabilities'],'callable_frontend_id':cb.callable_receipt['callable_frontend_id'],'call_count':cb.callable_receipt['call_count'],'specializations':cb.callable_receipt['specializations'],'optimization_id':tool['artifact'].optimization_receipt['optimization_id'],'codegen_id':tool['artifact'].codegen_receipt['codegen_id'],'gvm_sha256':tool['artifact'].codegen_receipt['gvm_sha256'],'equivalence':tool['equivalence']['status'],'halted':bool(execution.halted) if execution else None}
        receipt=dict(pre); receipt['package_build_id']='gcpkg-'+sha256_obj(pre)[:24]
        return CallablePackageBuildResult(root,lock,cb,tool,execution,receipt,source_packages)
    def write_build(self,result,outdir):
        out=Path(outdir); out.mkdir(parents=True,exist_ok=True); result.lockfile.write(out/'genesis.lock.json')
        (out/'program.gvm').write_bytes(result.toolchain['artifact'].bytecode)
        write_json(out/'PACKAGE_BUILD_RECEIPT.json',result.receipt)
        write_json(out/'CALLABLE_FRONTEND_RECEIPT.json',result.callable.callable_receipt)
        write_json(out/'CALLABLE_PROOFS.json',result.callable.callable_proofs)
        write_json(out/'CALL_EXPANSIONS.json',[x.as_dict() for x in result.callable.expansion_records])
        ...
        ex=out/'expanded'; ex.mkdir(exist_ok=True)
        for f,t in result.callable.expanded_sources: (ex/Path(f).name).write_text(t,encoding='utf-8')
        return out
